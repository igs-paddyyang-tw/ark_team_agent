...
> 發布了**（`7b68c43` session 監控）。改標 1.8.8，不搶版號。
> 今天第六次 —— 已成常態，發版前一律先問 Release API。

### 🔴 實機觀察真實 TG 卡片 —— 長路徑從左截，檔名被切掉

第一次讀到**真實 TG 上那則訊息**（nana-team 的 `librarian-agent`，
用 `forwardMessage` 取回文字後刪除轉發副本）：

```
📚 羅賓（圖書館長） ✅ 完成
📋 請只做這三件事…（prompt 原文，截 100 字）

✅ 已完成（3 步驟）
　🔍 分析 知識庫                                        ← wiki_query（MCP）✅
　⚙️ 執行 ls                                            ← shell（內建）✅
　🔍 分析 /home/paddyyang/kiro-cli/projects/nana-team-age⋯  ← 🔴 檔名被切掉
```

前兩項證實 1.8.2–1.8.4 的修正在真實 TG 上有效。
第三項的 `AGENTS.md` **整個被切掉** —— **那一行等於沒有資訊**。

> 💡 **短樣本測不出截斷方向。** 我的 fixture 用 `sample.txt` / `out.txt`，
> 永遠碰不到 `_OBJ_CAP`。這是「先觀察實機」抓到的第二個缺口
> （第一個是 1.8.4 的 spinner 前綴）。

### 修法：路徑留尾，指令留頭

| 形狀 | 判準 | 截斷方向 | 理由 |
|---|---|---|---|
| 路徑 | 含 `/` **且無空白** | **留尾**（`⋯/nana-team-agent/AGENTS.md`） | 有資訊的是檔名，在最後 |
| 指令 | 含空白 | 留頭（`ls -la /home/…⋯`） | 有資訊的是動作，在開頭 |
| 其他 | 無 `/` | 留頭 | 查詢字串等同理 |

⚠️ 判準刻意**不是**「含 `/` 就留尾」—— `ls -la /home/...` 也含 `/`，
而它留尾會變成一串路徑尾巴，看不出在做什麼（已釘反證）。

### 順帶記錄：完成卡片什麼時候才看得到

`finalize()` 只在兩個時機觸發：**下一段 stdout 到來**（段落邊界，
之後 `soft_reset` 保留 message_id 繼續 edit 同一則）或 **orphan
收尾**（180s 久未更新且沒有 reply）。

而 `reply()` 會把**同一則訊息** edit 成答案（`api.py`）。所以：

- 正常成功路徑 → 卡片被答案**取代**，使用者看到的是答案
- **沒有 reply** → 卡片停在完成狀態 ← **使用者回報的就是這個**

本次刻意派一個「不要呼叫 reply」的任務來重現它。

### 測試

53 條（+6）。全量 **3199 passed**，反證 2 項全紅。
...
① **原值與 `_DONE_MAX_ITEMS`（12）撞值** —— 12 步的任務會同時吃到
   「完整 12 項清單」與「Skill 提示」，卡片一下變很長。
② **16 > `skill_growth._DEFAULT_TOOL_THRESHOLD`（15）** —— 卡片印提示時，
   真正會發建議訊息的機制**必定已經越過自己的門檻**。
   原本 12 < 15，卡片會在真機制還沒觸發時就先喊，兩者說法不一致。

### ⚠️ 三個門檻語意不同，不要「統一」它們

盤點時發現「幾步算多」在三個地方各有一個值 —— 而它們管的是
**成本遞增的三個動作**，值不同是刻意的：

| 常數 | 值 | 動作 | 附加條件 |
|---|--:|---|---|
| `skill_extractor._DEFAULT_MIN_STEPS` | 5 | 自動萃取 skill | 2h cooldown |
| `tool_tracker.SKILL_HINT_STEPS` | **16** | 卡片印一行提示 | 無 |
| `skill_growth._DEFAULT_TOOL_THRESHOLD` | 15 | **真的發一則建議訊息** | diversity ≥0.5 + 1h cooldown |

> 💡 值可以各自調，但**順序有意義**：提示的門檻不該低於「真的發訊息」的門檻，
> 而最不干擾的動作（自動萃取）該是門檻最低的。
> 已加守門把這三條不變式釘住 —— 否則下一個人會以為它們該被統一成一個常數。

`_KEEP_STEPS`（24）的說明一併更正（原寫「≈2× `SKILL_HINT_STEPS`」，
現在是「> 16，且提示出現時清單不會已經被截」，也有守門）。

### 測試

47 條（+5）。行為層兩個方向都驗（15 步不提示、16 步提示）。
全量 **3193 passed**，反證改回 12 → 3 條紅。

## 1.8.5 (2026-09-11)

### 完成卡片的截斷上限：把「保留」與「顯示」分開

1.8.4 之後長任務仍會被截（誠實地說「僅顯示最後 8 項」，但 8 太少）。
根因是 `_MAX_LINES = 8` **同時決定保留多少與顯示多少** ——
而 `_steps` 在**收集階段**就被截掉 → 完成卡片想多顯示也沒有資料。

> 🔴 這是 1.8.2 剛修過的同一個形狀：**一個變數服務兩個需求相反的畫面。**
> 工作中／detail 模式要**少**（每 3 秒 edit 一次，工具牆是噪音）；
> 完成卡片要**多**（那是最後一眼）。

實測步數分佈（`state/tool_calls.log`，同 agent 間隔 >180s 切成一個任務，
501 個任務。⚠️ **只含 MCP 呼叫**，內建工具另計 → 真實步數更高）：

| ≤4 步 | ≤7 步 | ≤10 步 | ≤12 步 | 離群 |
|---|---|---|---|---|
| 84% | 90% | 95% | 99% | 21 / 22 / 68 |

拆成三個上限：

| 常數 | 值 | 管什麼 |
|---|--:|---|
| `_KEEP_STEPS` | **24** | 保留幾個 step（≈2× `SKILL_HINT_STEPS`，涵蓋內建工具翻倍後的長任務） |
| `_DONE_MAX_ITEMS` | **12** | 完成卡片最多列幾項（相鄰摺疊**之後**才套） |
| `_MAX_LINES` | 8 | detail 模式每次 edit 顯示幾行（不變） |

代價：24 × 約 100B = 2.4KB／tracker；TG 12 × 約 60 字 ≈ 720 字（上限 4096）。

### 🔴 說一半的截斷警告，跟不說一樣會誤導

兩種截斷**可以同時發生**。第一版用 `if/elif` → 40 步時只顯示
「保留最後 24 步」而畫面**只有 12 行**，讀的人會想不通少的那 12 行去哪了。

改成兩個獨立的 `if` 並合併成一句：

```
  5 步 → ⋯（無截斷說明，正確）
 14 步 → ⋯ 顯示最後 12 項（共 14 項）
 40 步 → ⋯ 共 40 步，保留最後 24 步，顯示最後 12 項
```

### 顯示上限套在摺疊之後

先截再摺會讓上限吃掉本來能壓進去的內容 ——
30 次相同的 `⚙️ 執行 ls` 摺成 1 項後不該觸發顯示截斷（已釘測試）。

### 測試

42 條。既有 `test_max_lines` **更新語意**（原本斷言「保留量 ≤ 8」，
現在分別釘住「保留有上限」與「detail 顯示有上限」）——
不是把斷言放寬：退回單一上限的話，保留截斷與卡片內容都會變。
全量 **3187 passed**，反證 4 項全紅。

⚠️ 順帶：`test_tool_tracker.py` 是 **CRLF** 檔，`write_text` 把它正規化成 LF
→ `test_line_endings_preserved` 抓到（本 repo 記過的第五次）。已用 bytes 層級還原，
`git diff` 與 `-w` 版本都是 19+/5-（零假變更）。
...
One-pager：`docs/one-pagers/2026-09-09-team-memory-flow-home.md`

| | bot 側 | team 側 |
|---|---|---|
| 自動寫 `memory/daily/` | ✅ `write_daily_log()` | 🔴 **完全沒有** |
| agent 能寫 `memory/` 的 MCP 工具 | — | 🔴 **12 個工具裡零個** |
| 歸檔／掃機密 `memory/daily/` | ✅ | ✅ —— **對著沒有寫入端的目錄工作** |

而 `SOUL.md` 寫著「每完成一個段落更新 `MEMORY.md`」——
那是 agent **唯一寫得到的地方**。

實測 `aiops/obs-agent`：`log-patrol` 每小時一次 → **410 筆 / 278 KB**
累積在一個 `## 近期進度` 底下，注入量 **388,754**（hard limit 250K，超 55%）
—— 比 paddy 曾經的 321K 更嚴重，而 **preflight P0/P1 全 0**
（context 只被量測不被探測）。

### 🔴 1.7.27 的位元組預算救不了它 —— 粒度不同

歸檔器的判準是 `^## YYYY-MM-DD` 的**分節**，而巡檢的日期在
`- 2026-09-07 00:30：` 的**條目層** → 看不到任何可歸檔的東西
（它正確地停下並 warning，不硬砍近期內容）。

### 判準：抓「排程節奏」—— 我試錯了兩次

| 版 | 判準 | 誤抓 |
|:-:|---|---|
| 1 | 條目式（`- YYYY-MM-DD：`） | `ninja` 的 44 筆真記憶（「skill 正名」「三個安靜失敗 PR #8」）|
| 2 | 帶時刻 ∧ 同日 ≥4 筆 | 🔴 `sre` 的 `18:00/18:35/18:39/18:40` —— **P1 事故當下的連續回報**，而它的 SLO 日報反被判成記憶 |
| **3** | ＋**分鐘規律**（≥75% 同分鐘，或分鐘只有 ≤4 種且全是 15 倍數） | 零誤報 |

> 💡 **排程的指紋是「分鐘固定」，事故的指紋是「分鐘散亂」** ——
> 兩者同一天都可能有很多筆，只有分鐘分佈能分開它們。

每個判定附**可複核的理由**（`24/24 筆在 :30（100%）`）——
我兩次判錯都是靠「看它抓到什麼」發現的。

### 🔴 `keep_days` 也錯了一次 —— 同一條教訓的第三次

`keep_days=3` 實測對 obs **搬 0 筆**（87 筆全在最近 3 天內）：

| keep_days | 3 | 2 | 0 |
|---|--:|--:|--:|
| 搬 | **0 筆** | 24 | 72 |

**天數不控制量，頻率才控制量** —— 每小時一筆的 agent 3 天 = 72 筆，
每 6 小時一筆的 3 天 = 12 筆，同一判準效果差 6 倍。
改 `keep_recent=12`（筆數）。這是 1.7.27 的 `max_bytes` 學過的同一條：
**約束是量，判準就不能是時間。**

副效果正好：`ic`（9 筆）與 `mlops`（12 筆）都 ≤ `keep_recent` → **不動**，
只有真的有量的 obs 被處理。

### 實測結果

```
obs-agent  388,754 [hard] ──手動搬 324 筆──▶ 182,220 [soft] ──memory_flow 75 筆──▶ 124,272 [ok]
           MEMORY.md 281,745 → 17,263B
```

帳目核對：原檔 95 條目 = 留 20 ∪ 搬 75，**遺失 0、重複 0**。

### 接線

`_builtin:memory-consolidate`（每週日 03:30）**先搬流水、再做分節歸檔**
—— 順序是硬的（歸檔器看不到條目層）。摘要新增「搬出 N 筆排程流水」。

### 守門

`tests/test_memory_flow.py` **33 條**（含兩個病例回放），反證 **13 項**各紅。
⚠️ CRLF 寫入端在 Linux 上測不出來（`os.linesep` = `\n`）→
改用 ast 掃「每個 `open()` 都要有 `newline=""`」釘住，
**不假裝行為測試涵蓋了它**。

過程中三個錯全被測試抓到：測試檔漏 import、改判準時移除了 `today` 的定義
（**`ast.parse` 通過、import 通過，只有真的呼叫才炸**）、
以及 `test_pointer_is_idempotent` 第一版是空的
（第二次跑時 `move` 為空就 return，**走不到指標段那條路**）。
